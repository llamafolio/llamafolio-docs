...
Important note: For performance reasons (ex: fetching 1 million pools), `getContracts` is executed on a regular basis, not during the update balance process.
...
To get reserves info, let's iterate on all reserve contract (using `getReservesList`) and call `getReserveData`.
