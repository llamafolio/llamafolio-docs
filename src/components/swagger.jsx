...
/**
 * Fetch latest spec from `llamafolio-api` repo or fallback in case of error
 */
async function fetchSpec() {
  try {
    const latestSpecResponse = await fetch(
      "https://raw.githubusercontent.com/llamafolio/llamafolio-api/master/docs/swagger/swagger.json"
    );

    if (!latestSpecResponse.ok) {
      throw new Error(`Failed to fetch latest spec`);
    }

    return latestSpecResponse.json();
  } catch {
    return spec;
  }
}
